...
title_graphics("The Hero's Journey")
...
def character_creator():
    print(
        Fore.CYAN
        + f"1: Warrior, 2: Mage, 3: Thief, 4: Demonknight, 5: Bard, 6: Priest"
        + Style.RESET_ALL
    )
    while True:
        try:
...
def merchant_event():
    ...
    if m_choice in ["y", "yes"] and player["gold"] >= price:
        ...
        if merch == "armorkit":
            player["armor"] += 0.03 + player["repair_armor_bonus"]
            if player["armor"] > player["max_armor"]:
                player["armor"] = player["max_armor"]
            print(
                merch_msgs["armorkit"],
                f"{int((.03 + player['repair_armor_bonus']) * 100)}% armor",
            )
        if merch == "whetstone":
            player["damage"] += 4
...
def attack(unit, target):
    ...
    if block <= target["block_chance"]:
        print(
            Back.BLUE
            + f"\n--- {target['name']} blocked {unit['name']}'s attack!:"
            + Style.RESET_ALL
        )
        ...
        if unit["lifesteal"] is True:
            healing(
                unit, round(armor_based_dmg / unit["ls_coefficient"]), " by vampirism"
            )
...
def fireball_attack(unit, target):  # isolated
    if unit["fireballs"] > 0:
        ...
        print(
            Fore.BLACK
            + Back.WHITE
            + f"\nYou don't have fireball scrolls!"
            + Style.RESET_ALL
        )
...
def acidbomb():
    if "acidbomb" in player_inventory:
        ...
        print(
            ...
            f"enemy's armor destroyed and {player['bomb_dmg']} pure dmg dealt:"
            + Style.RESET_ALL
        )
        status(player, enemy, undead_list, boss_list)
    else:
...
def slimebomb():
    if "slimebomb" in player_inventory:
        ...
        print(
            ...
            f"enemy's dmg decreased to 1/3 and {player['bomb_dmg']} pure dmg dealt:"
            + Style.RESET_ALL
        )
        status(player, enemy, undead_list, boss_list)
    else:
...
def loot():
    ...
    if (
        sword_drop_chance == 10
        and all(x not in player_inventory for x in ["demonblade", "sword"])
        and ic("sword")
        and enemy["name"] in ["Orc", "Assassin", "Marauder"]
    ):
        player_inventory.append("sword")
        player["damage"] += 15
        print(loot_msgs["sword"])
    if (
        armorpants_drop_chance == 10
        and "armorpants" not in player_inventory
        and ic("armorpants")
    ...
    if (
        luckycoin_drop_chance > 8
        and enemy["name"] == "Gnome"
        and "luckycoin" not in player_inventory
        and ic("luckycoin")
    ):
    ...
    if (
        rosary_drop_chance > 8
        and enemy["name"] == "Crusader"
        and "rosary" not in player_inventory
        and ic("rosary")
    ):
    ...
    if (
        coolhat_drop_chance == 10
        and enemy["name"] == "Troll"
        and "coolhat" not in player_inventory
        and ic("coolhat")
    ):
    ...
    if (
        7 < potion_drop_chance < 10
        and enemy["name"] != "Zombie"
        and player["allow_potions"] is True
    ):
        player["healing_potions"] += 1
        ...
        print(
            loot_msgs["stew"],
            f"and max health +{10 + player['stew_bonus']} ({player['max_health']} now)",
        )
    if armor_event_chance in [1, 2]:
        player["armor"] += 0.02 + player["repair_armor_bonus"]
        if player["armor"] > player["max_armor"]:
            player["armor"] = player["max_armor"]
        print(
            loot_msgs["armor_up"],
            f"+{int((.02 + player['repair_armor_bonus']) * 100)}% armor",
        ...
        if player["armor"] < 1.0:
            player["armor"] = 1.0
        print(
            Back.RED + Fore.WHITE + loot_msgs["armor_down"],
            f"({round(player['armor'], 2)} now)" + Back.WHITE + Fore.BLACK,
        )
    if training_event_chance == 1:
        player["damage"] += 2 + player["training_bonus"]
        print(
            loot_msgs["training_dmg"],
            f"{2 + player['training_bonus']} dmg! ({player['damage']} now)",
        )
        ...
        player["crit_mul"] += 0.05 + player["crit_training_bonus"]
    ...
    if 3 < fireball_drop_chance < 9 and enemy["name"] in ["Warlock", "Revenant"]:
        ...
        fireball_drop_chance > 8
        and enemy["name"] == "Warlock"
        and "firebook" not in player_inventory
        and ic("firebook")
    ):
    ...
    if (
        stonemask_drop_chance > 7
        and enemy["name"] == "Vampire"
        and ic("stonemask")
        and all(x not in player_inventory for x in ["stonemask", "soulring"])
        and player["class"] != "Priest"
    ):
        sm_cons = input(
            "You've found Stonemask. Do you want to take it? (Demonic item)"
        )
        if sm_cons in ["yes", "y"]:
    ...
    if (
        demonblade_drop_chance > 8
        and enemy["name"] == "Marauder"
        and "demonblade" not in player_inventory
        and player["class"] != "Priest"
        and ic("demonblade")
    ):
        db_cons = input(
            "You've found Demonblade. Do you want to take it? (Demonic item)"
        )
        if db_cons in ["yes", "y"]:
    ...
    if (
        shield_drop_chance > 8
        and enemy["name"] == "Orc"
        and "shield" not in player_inventory
        and ic("shield")
    ):
    ...
    if ghost_debuff_chance < 3 and enemy["name"] == "Ghost":
        ...
        if player["health"] > player["max_health"]:
            player["health"] = player["max_health"]
        print(
            Back.RED + Fore.WHITE + loot_msgs["ghost_debuff"] + Back.WHITE + Fore.BLACK
        )
    if (
        speedboots_drop_chance > 8
        and enemy["name"] == "Bandit"
        and "speedboots" not in player_inventory
        and ic("speedboots")
    ):
    ...
    if (
        enemy["name"] == "Warchief"
        and "wristbands" not in player_inventory
        and ic("wristbands")
    ):
    ...
    if (
        ...
        and "soulring" not in player_inventory
        and player["class"] != "Priest"
        and ic("soulring")
    ):
    ...
    if (
        enemy["name"] == "Alchemist"
        and "herbkit" not in player_inventory
        and player["class"] != "Demonknight"
        and ic("herbkit")
    ):
    ...
    if (
        gloves_drop_chance > 8
        and enemy["name"] == "Assassin"
        and "gloves" not in player_inventory
        and ic("gloves")
    ):
...
def ic(new_item):
    if len(player_inventory) >= player["inv_size"]:
        print(Style.RESET_ALL, end="")
        question = input(
            f"Here {new_item.capitalize()}, but your inventory is full. "
...
def persuading(hero, opponent, container):
    ...
    if (
        pe_chance < hero["persuade_chance"]
        and any(opponent["name"] not in ls for ls in [undead_list, boss_list])
        and player["class"] != "Demonknight"
        or pe_chance < hero["persuade_chance"]
        and opponent["name"] not in boss_list
        and player["class"] == "Demonknight"
    ):
        print(
            Fore.GREEN
...
def bribing(hero, opponent, container):
    if hero["gold"] >= opponent["bribe_value"] and any(
        opponent["name"] not in ls for ls in [undead_list, boss_list]
    ):
...
while True:
    try:
        ...
        # split() for multi-word commands
            ...
            action = action.split(" ")
            if (
                action[1] in syntax_dict["attack_list"]
                and action[0] in syntax_dict["player_list"]
            ):
                ...
                action[0] in syntax_dict["fireball_list"]
                and action[1] in syntax_dict["attack_list"]
            ):
                ...
                action[0] in syntax_dict["acidbomb_list"]
                and action[1] in syntax_dict["attack_list"]
            ):
                acidbomb()
                if enemy["health"] < 1:
                ...
                action[0] in syntax_dict["slimebomb_list"]
                and action[1] in syntax_dict["attack_list"]
            ):
                slimebomb()
                if enemy["health"] < 1:
                ...
                action[0] in syntax_dict["smokebomb_list"]
                and action[1] in syntax_dict["attack_list"]
            ):
                smokebomb()
                if enemy["health"] < 1:
                ...
                action[0] in syntax_dict["banisher_list"]
                and action[1] in syntax_dict["attack_list"]
            ):
                ...
                action[0] in syntax_dict["soulorb_list"]
                and action[1] in syntax_dict["attack_list"]
            ):
                ...
                action[1] in syntax_dict["inventory_list"]
                and action[0] in syntax_dict["inspect_list"]
            ):
                show_inventory(
                    player, player_inventory, player["inv_size"], equipment_desc
                )
                status(player, enemy, undead_list, boss_list)
            elif (
                action[1] in syntax_dict["attribute_list"]
                and action[0] in syntax_dict["inspect_list"]
            ):
                show_attributes(player, player_attributes, attr_desc)
                status(player, enemy, undead_list, boss_list)
            elif (
                action[1] in syntax_dict["player_list"]
                and action[0] in syntax_dict["inspect_list"]
            ):
                ...
                action[1] == "up"
                and action[0] in player_attributes.keys()
                or action[1] == "up"
                and action[0] in lowcase_player_attr.keys()
            ):
                attribute_up(player, player_attributes, action[0].upper(), attr_desc)
                status(player, enemy, undead_list, boss_list)
            elif (
                action[1] in player_inventory and action[0] in syntax_dict["drop_list"]
            ):
                drop_item(action[1])
                show_inventory(
                    player, player_inventory, player["inv_size"], equipment_desc
                ...
                action[1] in syntax_dict["enemy_list"]
                and action[0] in syntax_dict["inspect_list"]
            ):
                if enemy["mod"] != "":
                    inspect_enemy(
                        enemy["name"],
                        enemies_desc[enemy["name"]],
                        enemy["mod"],
                        modification_desc[enemy["mod"]],
                    )
                else:
